...
*/
-- 데이터베이스(스키마) 권한 부여
GRANT ALL PRIVILEGES ON kh.* TO test; -- 전체 권한

-- 데이터베이스(스키마) 권한 회수
REVOKE ALL PRIVILEGES ON *.* FROM test; -- Workbench가 인지를 못해서 오류로 보일뿐. 실행은 정상
...
-- 스키마 삭제
-- DROP SCHEMA sample;
DROP SCHEMA IF EXISTS sample; -- 존재하는 경우에 삭제하겠다. sample이 있는 경우만 삭제하고 없으면 실행을 안 하는 것
