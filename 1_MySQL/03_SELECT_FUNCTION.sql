...
*/
SELECT '데이터베이스';
SELECT length('데이터베이스') 'BYTE', char_length('데이터베이스') '글자 수',
...
SELECT
	emp_name,	
    email '변경 전 이메일',
    replace(email, 'kh.or.kr', 'gmail.com') '변경 후 이메일'
FROM employee;
...
FROM employee;
-- 내가 짠 코드 ↑replace, ↓rpad 오류
-- SELECT emp_name, rpad(emp_no, substr(emp_no, 9, 14),'*') FROM employee;
...
-- rpad 해설
-- SELECT emp_name, rpad(emp_no, 몇글자인지, 채울글자) 
...
-- substr, replace, instr, trim
-- 내가 짠 코드. 오류
SELECT emp_name, email, substr(email, instr(email,'@'))
FROM employee;
...
/*
    ...
    - 반올림한 결과를 반환
*/
SELECT round(123.456), round(123.567, 2), round(123.567, -1); -- 123  123.57  120

/*
	CEIL : 올림 처리해서 반환
    FLOOR : 버림 처리해서 반환
*/
SELECT ceil(123.152), floor(123.152); -- 124  123

/*
	TRUNCATE(숫자, 위치)
    - 위치 지정하여 버림 처리해서 반환
*/
SELECT truncate(123.456, 1), truncate(123.456, 0), truncate(123.456, -1);
-- 123.4 소수점까지 / 123(소수점 없애기. but, 이렇게 쓰려면 floor 사용) / 120
...
*/
SELECT dayofyear(now()), dayofmonth(now()), dayofweek(now()); -- 171  19  4
...
SELECT datediff('2024-12-31', now()), timediff('2025-01-01 00:00:00', now()); -- 195
...
SELECT
    ...
    addtime(now(), "01:10:00"), -- 더하고 싶은 만큼 더할 수 있음
...
/*
    ...
    - 조건에 해당하면 두 번째 값 반환, 해당하지 않으면 마지막 값 반환
*/
...
-- SELECT emp_id, emp_no, if(substr(emp_no, 8, 1) IN (1, 3), '남', '여') 성별
SELECT
...
-- if(job_code = 'J7', salary * 1.1, if()) >> if(,, if()) 이렇게 처리해야 함
-- if(job_code = 'J7', salary * 1.1)
-- if(job_code = 'J6', salary * 1.1) 이렇게 if를 계속 적어서 처리하면 완전히 다른 쿼리문이 됨
...
SELECT
	...
	case when salary > 5000000 then '1등급'
		-- when salary between 5000000 and 3500000 then salary = '2'
        -- 위에서 500만원 초과가 걸러졌기 때문에. 굳이 길게 코딩 안 해도 됨. 조건 >> salary = '2' X. '2등급'!
		-- when salary between 3500000 and 2000000 then salary = '3' 굳이
...
/*
	그룹함수 --> 결과값 1개!
    - 대량의 데이터들로 집계나 통계 같은 작업을 처리해야 하는 경우 사용하는 함수들
    - 모든 그룹 함수는 NULL 값을 자동으로 제외하고 값이 있는 것들만 계산
    
    SUM : 해당 컬럼 값들의 총 합계 반환
*/
-- 전체 사원의 총 급여 합 조회
SELECT sum(salary) -- 다른 애들 같이 못 쓴다?
FROM employee;
...
select emp_name, dept_code, format(salary, 0)
from employee
where dept_code = 'd5';
...
FROM employee;
-- 3047662.6087  0.21666666666666667  3047662.6087  0.08478260869565218
-- avg(ifnull(salary, 0)), avg(ifnull(bonus, 0)) 값이 다른 이유
-- avg는 null값 자동 제외, bonus는 null 값을 제외하지 않고 0으로 처리해서 평균값이 구해져 달라짐

/* 숫자 계산이 아니기 때문에 모든 컬럼 값들 다 사용 가능
