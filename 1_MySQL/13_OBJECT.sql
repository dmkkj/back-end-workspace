...
/*
    ...
    DELIMITER // -- 시작하겠다. 여기부터 프로시저 시작한다
...
-- DELETE 데이터 삭제, DROP 객체 삭제. 함수는 객체
-- 함수 삭제
...
-- employee 테이블에서 UPDATE 수행 후 '업데이트 실행'
-- 메세지를 log에 담는 trg_02 트리거 생성
...
/*
    ...
    1. 개념적 모델링
		- 엔티티(Entity) 추출
        - 엔티티 간의 관계 설정
    2. 논리적 모델링 : ERD(Entity Relationship Diagram) 툴
		- 정규화 작업 (1 ~ 5) ... 3까지만 정규화!
		  --> 너무 쪼개면 join만 많아져요!
    3. 물리적 모델링
		- 테이블 실질적으로 구성
