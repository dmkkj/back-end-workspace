...
//		System.out.println(video.allVideo(paging));
	...
	@ResponseBody
	@GetMapping("/list")
	public List<Video> list(Paging paing) {
		return video.allVideo(paing);
	}
	
	// 비디오 1개 보여주기 // http://localhost:8080/watch?v=2
	// 좋아요 관련 정보 가져오기
	// 구독자수, 구독 관련 정보 가져오기
	...
	public String detail(@PathVariable("videoCode") int videoCode, Model model, Paging paging) {
		...
		model.addAttribute("video", data); // model.addAttribute("video", video.detail(videoCode));
		model.addAttribute("list", video.allVideo(paging));
