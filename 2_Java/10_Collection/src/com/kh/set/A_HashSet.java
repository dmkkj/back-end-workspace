...
public class A_HashSet {
	...
	public void method1() {
		...
		System.out.println(set.size()); // 리스트 안에 몇 개의 데이터가 있는지
		System.out.println("구교환이 포함되어 있는가? " + set.contains("구교환"));
		
		set.remove("홍사빈");	// list 삭제 메서드
		System.out.println(set);
		
		set.clear();
		System.out.println("비어있는지? " + set.isEmpty()); // 리스트가 비어있냐
	}
	...
	public void method2() {
		...
		// com.kh.set.model > Person.java > public int hashCode() { return Objects.hash(age, name); } 추가 시 중복 제거
		System.out.println(set);
