...
public class A_ArrayList {
	...
	public void method1() {
		...
		list.add("장영태"); // .add() 메서드
		list.add("장성일");
		list.add("채승훈");
		list.add("손정배");
		list.add("김진주");
//		list.add(1); // ArrayList<String> 타입만 올 수 있어 에러 
//		list.add(2);
//		list.add(3);
		
		System.out.println(list); // 배열은 Arrays.toString() 처리해야 하지만(안 하면 그냥 주소값 출력됨) 얘는 그냥 가능
	}
	...
	public void method2() {
		...
		// 1. add : 리스트 끝에 추가
		list.add(new Person("전현무", "삼성동", 46)); // 명시한 타입만 올 수 있음. <Person>으로 지정해서 .add(new Person());
		...
		List<Person> sub = list.subList(0, 2); // 0부터 2까지
		...
		// 9. isEmpty : 컬렉션이 비어있는지 (true / false로 리턴)
		...
		// 삼성동에 사는 사람들만 출력
		for(Person p : list) {
			if(p.getAddr().equals("삼성동")) {	// 문자열끼리 비교할 때 필요한 것 : .equals("문자열")
			System.out.println(p);
			}
		}
		
		// 나이 순서대로 출력 -> 이름 순서대로 출력 (Person.java > public int compareTo(Person o) {})
